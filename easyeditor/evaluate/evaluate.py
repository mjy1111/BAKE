...
def compute_icl_edit_quality_bi(
        model,
        model_name,
        hparams: HyperParams,
        tok: AutoTokenizer,
    ...
    prompt = record["prompt"]
    rephrase = record["rephrase_prompts"] if 'rephrase_prompts' in record.keys() else None
    new_fact = f'New Fact: {prompt} {target_new}\nPrompt: {prompt}'

    reverse_qa_prompts = record["reverse_qa_prompts"]["prompt"] if 'reverse_qa_prompts' in record.keys() else None
    reverse_qa_target_new = record["reverse_qa_prompts"]["target_new"]["str"] if 'reverse_qa_prompts' in record.keys() else None

    reverse_judge_prompts = record["reverse_judge_prompts"]["prompt"] if 'reverse_judge_prompts' in record.keys() else None   
    ...
    if "reverse_qa_prompts" in record:
        rqa_ret = icl_lm_eval(
            model, model_name, hparams, tok, icl_examples,
            reverse_qa_target_new, 
            f'New Fact: {reverse_qa_prompts} {reverse_qa_target_new}\nPrompt: {reverse_qa_prompts}'
        )
        reverse_res['reverse_qa'] = [rqa_ret]

    if "reverse_judge_prompts" in record:
        rj_ret = evaluate_reverse_judge(
            model, model_name, hparams, tok, icl_examples,
            prompt=reverse_judge_prompts,
            expected_answer="yes"
        )
        reverse_res['reverse_judge'] = [rj_ret]

    if reverse_res:
        ret["reverse_evaluation"] = reverse_res
...
def compute_edit_quality_bi(
    ...
    rewrite_prompts = record["prompt"]
    rephrase_prompts = record["rephrase_prompts"]
    ret = compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                              rewrite_prompts, target_new, device=device, eval_metric=eval_metric)
    ...
    reverse_res = {}
    if "reverse_qa_prompts" in record:
        rqa_ret = compute_reverse_qa_quality(
            model, model_name, hparams, tok,
            reverse_qa_record=record["reverse_qa_prompts"],
            device=device,
            eval_metric=eval_metric
        )
        reverse_res.update(rqa_ret)

    if "reverse_judge_prompts" in record:
        rj_ret = compute_reverse_judge_quality(
            model, model_name, hparams, tok,
            record=record,
            device=device,
            eval_metric=eval_metric
        )
        reverse_res.update(rj_ret)
...
def compute_reverse_qa_quality(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    reverse_qa_record: dict,
    device,
    eval_metric: str = 'token_em'
) -> typing.Dict:


    qa_prompt = reverse_qa_record["prompt"]
    new_str = reverse_qa_record["target_new"]["str"]

    if hasattr(hparams, 'evaluation_type') and hparams.evaluation_type == "LLM-judge":
        acc, gen_content = test_prediction_acc_LLM_judge(model, tok, hparams, qa_prompt, new_str, locality=False)
        ret = {
            "reverse_qa_acc": acc,
            "reverse_qa_gen_content": gen_content
        }
    else:
        if eval_metric == 'ppl':
            ppl = PPL(model, tok, qa_prompt, new_str, device)
            ret = {
                "reverse_qa_ppl": ppl
            }
        elif eval_metric == 'ood_ppl':
            ans = OOD_PPL(model, tok, qa_prompt, new_str, device)
            ret = {
                "reverse_qa_ood_acc": ans
            }
        elif hparams.alg_name == "GRACE":
            if 't5' in model_name.lower():
                acc = test_seq2seq_batch_prediction_acc(model, tok, hparams, qa_prompt, new_str, device)
            else:
                acc = test_prediction_acc(model, tok, hparams, qa_prompt, new_str, device, vanilla_generation=True)
            f1 = F1(model, tok, hparams, qa_prompt, new_str, device, vanilla_generation=True)
            ret = {
                "reverse_qa_acc": acc,
                "reverse_qa_f1": f1
            }
            ...
            if 't5' in model_name.lower():
                ...
                acc = test_prediction_acc(model, tok, hparams, qa_prompt, new_str, device)
            ret = {
                "reverse_qa_acc": acc
            }
...
def compute_reverse_judge_quality(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    judge_prompt = record["reverse_judge_prompts"]["prompt"]
    final_new_str = record["reverse_judge_prompts"]["target_new"]["str"]
    

    if hasattr(hparams, 'evaluation_type') and hparams.evaluation_type == "LLM-judge":
        acc, gen_content = test_prediction_acc_LLM_judge(model, tok, hparams, prompt=judge_prompt, target_new=final_new_str, device=device, locality=False)
        ret = {
            "reverse_judge_acc": acc,
            "reverse_judge_gen_content": gen_content
        }
    else:
        if eval_metric == 'ppl':
            ppl_val = PPL(model, tok, judge_prompt, final_new_str, device)
            ret = {
                "reverse_judge_ppl": ppl_val
            }
        elif eval_metric == 'ood_ppl':
            ans = OOD_PPL(model, tok, judge_prompt, final_new_str, device)
            ret = {
                "reverse_judge_ood_acc": ans
            }
        elif hparams.alg_name == "GRACE":
            if 't5' in model_name.lower():
                acc = test_seq2seq_batch_prediction_acc(model, tok, hparams, judge_prompt, final_new_str, device)
            else:
                acc = test_prediction_acc_judge(model, tok, hparams, judge_prompt, final_new_str, device, vanilla_generation=True)
            f1_val = F1(model, tok, hparams, judge_prompt, final_new_str, device, vanilla_generation=True)
            ret = {
                "reverse_judge_acc": acc,
                "reverse_judge_f1": f1_val
            }
        else:
            if 't5' in model_name.lower():
                acc = test_seq2seq_batch_prediction_acc(model, tok, hparams, judge_prompt, final_new_str, device)
            else:
                acc = test_prediction_acc_judge(model, tok, hparams, judge_prompt, final_new_str, device)
            ret = {
                "reverse_judge_acc": acc
            }
...
def test_prediction_acc_judge(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False):
    ...
    if vanilla_generation:
        for prompt, target_new in zip(prompts, targets):
            ...
            gen_str = tok.decode(gen_token[0], skip_special_tokens=True).strip().lower()
            first_word = gen_str.split()[0] if len(gen_str.split()) > 0 else ""

            if first_word == target_new.strip().lower():
                results.append(1.0)
            else:
                results.append(0.0)
    else:
...
def evaluate_reverse_judge(
    ...
    input_text = ''.join(icl_examples) + prompt
    ...
    pred_token_id = torch.argmax(last_token_logits, dim=-1)
    pred_token = tokenizer.decode(pred_token_id)

    pred_token = pred_token.strip().lower()

    if "yes" in pred_token:
        pred = "yes"
    elif "no" in pred_token:
        pred = "no"
    else:
        pred = "unknown"

    return int(pred == expected_answer)